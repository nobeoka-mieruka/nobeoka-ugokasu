...
11. [LINEとInstagramを設定する方法](#11-lineとinstagramを設定する方法)
...
**公式アカウントのURL設定場所**：`src/data/socialLinks.ts` の `facebook` / `instagram` に公式アカウントのURLを設定してください。SNS投稿がまだ1件も登録されていない場合、活動報告ページに「活動の様子は、Facebook・Instagramでも随時発信しています。」という案内とあわせて、ここで設定したURLへのリンクボタンが自動的に表示されます（未設定の場合はボタン自体が表示されません）。
...
5-2の手動登録とは別に、FacebookページとInstagramプロアカウントへ新しく投稿すると、30分ごとに自動で活動報告ページへ反映される仕組みも用意されています。Meta Graph APIとCloudflareを使った仕組みで、アクセストークン等の秘密情報はCloudflareの暗号化されたSecretsにのみ保存し、GitHubやフロントエンドのコードには一切含まれません。

- 設定手順（Facebookページの作成〜トークンの更新方法まで）は [docs/social-sync-setup.md](./docs/social-sync-setup.md) にまとめています
- 設定が完了するまでは、この機能は自動的に「投稿0件」として扱われ、5-2の手動登録やサイトのデザインには一切影響しません
...
## 11. LINEとInstagramを設定する方法

`src/data/socialLinks.ts` を開き、該当するURLを入力してください。
