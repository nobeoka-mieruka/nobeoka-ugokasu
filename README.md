...
| `public/images/logo/` | ロゴ画像（未設定。用意でき次第配置） |
...
**公開内容を最終確認するまでは必ず `false` のままにしてください。** 本公開の準備が整ってから変更してください。
...
- [x] `robots.txt` が生成される
- [x] 下書きページ（`draft: true`、`published: false`）がnoindexになっている
- [x] 公開前は `allowIndexing: false` である
...
**SEOスコアについて**：唯一の減点理由は「Page is blocked from indexing」です。これは `allowIndexing: false`（[14. allowIndexingをtrueにする方法](#14-allowindexingをtrueにする方法)参照）により、公開準備が整うまで意図的に全ページをnoindexにしているためで、不具合ではありません。本公開時に `allowIndexing` を `true` に変更すると、この減点は解消される見込みです。
