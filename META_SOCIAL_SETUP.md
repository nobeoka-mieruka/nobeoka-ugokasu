...
4. 同じ設定画面の「KV namespace bindings」（Functionsの設定内）で、`SOCIAL_POSTS_KV` という名前でKV名前空間をバインドする（事前に Storage & Databases →「KV」から作成しておく）。このKVは、取得した投稿を約5分キャッシュし、API障害時に直前の投稿を表示し続けるために使用します（同じKVを、後述のYouTube動画一覧のキャッシュにも共用します。新しいKV名前空間を追加作成する必要はありません）。
